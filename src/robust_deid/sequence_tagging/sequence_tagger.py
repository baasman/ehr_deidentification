...
class SequenceTagger(object):
    def __init__(
        ...
        self._tokenizer_name = (
            tokenizer_name if tokenizer_name else self._model_name_or_path
        )
        ...
        self._model, self._post_processor = self.__get_model(
            threshold=threshold, quantize=quantize
        )
    ...
    def load(
        ...
        if label_all_tokens:
            ...
            for idx, label in enumerate(self._label_list):
                if (
                    label.startswith("B-")
                    and label.replace("B-", "I-") in self._label_list
                ):
                    b_to_i_label.append(
                        self._label_list.index(label.replace("B-", "I-"))
                    )
                else:
    ...
    def set_eval_metrics(
        ...
        validation_ids = [
            json.loads(line)["note_id"] for line in open(validation_spans_file, "r")
        ]
        validation_spans = [
            json.loads(line)["note_spans"] for line in open(validation_spans_file, "r")
        ]
        descriptions = [""]
        type_maps = [self._ner_types]
        if ner_types_maps is not None:
            descriptions += [
                "".join(list(set(ner_types_map) - set("O")))
                for ner_types_map in ner_types_maps
            ]
    ...
    def setup_trainer(self, training_args):
        ...
        self._trainer = Trainer(
            ...
            if self._metrics_compute is None
            else self._metrics_compute.compute_metrics,
        )
        checkpoint = None
    ...
    def predict(self, output_predictions_file: Optional[str] = None):
        if self._test_dataset is not None and self._trainer is not None:
            ...
            predictions, labels, metrics = self._trainer.predict(
                self._test_dataset, metric_key_prefix="predict"
            )
            unique_note_ids = set()
            ...
            true_predictions, true_labels = self._post_processor.decode(
                predictions, labels
            )
            note_predictions = note_level_aggregator.get_aggregate_sequences(
                sequences=true_predictions
            )
            note_labels = note_level_aggregator.get_aggregate_sequences(
    ...
    def __get_model(self, threshold: Optional[float] = None, quantize: bool = False):
        # Get the model
        ...
        if self._post_process == "argmax":
            model = model_picker.get_argmax_bert_model(quantize=quantize)
            ...
            model = model_picker.get_crf_bert_model(
                notation=self._notation, id_to_label=self._id_to_label
            )
...
def main():
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, EvaluationArguments, TrainingArguments)
    )
    ...
    if training_args.do_predict:
        sequence_tagger.predict(
            output_predictions_file=data_args.output_predictions_file
        )
